...
if __name__ == "__main__":
    ...
    if model_type == "ablang2":
        mutation_ids = pool_df["mutseq"].apply(lambda seq: [tokenizer_dict[s] - ABLANG2_TOKEN_OFFSET for s in seq]).values
        mutation_ids = torch.tensor(np.vstack(mutation_ids))

    elif model_type == "sequence":
        mutation_ids = pool_df["mutseq"].apply(lambda seq: [alphabet.tok_to_idx[s] - ESM_TOKEN_OFFSET for s in seq]).values
        mutation_ids = torch.tensor(np.vstack(mutation_ids))

    prediction_scores = get_prediction_score(mask_logits, wt_token_indices, mutation_ids)
    ...
    if ensemble_mode:
        ...
        ens_prediction_scores = [get_prediction_score(ens_logits, wt_token_indices, mutation_ids) for ens_logits in ens_logits_list]
