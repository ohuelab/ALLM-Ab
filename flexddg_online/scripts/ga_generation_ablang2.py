...
from logits import ablang2_logit_inference
...
def predict_score_ablang2(sequences):
    scores = ablang2_logit_inference(sequences, None, tokenizer, (wt_seq, light_wt_seq), (masked_wt_seq, light_wt_seq), [mask_indices, []], device="cuda", mask_logits=mask_logits)
    return scores
...
def evaluate_batch(individuals_batch):
    ...
    scores = predict_score_ablang2(batch_full_sequences)
...
if __name__ == "__main__":
    ...
    assert model_type =="ablang2", "model_type must be ablang2"
    ...
    masked_wt_seq = "".join([wt_seq[i] if i not in mask_indices else "*" for i in range(len(wt_seq))])
    ...
    masked_wt_seq = [masked_wt_seq, light_wt_seq]
